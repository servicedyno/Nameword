...
Continued the "Hostinger-inspired, media-rich" landing redesign. Real pod URL = `https://reseller-panel-9.preview.emergentagent.com` (both backend+frontend already RUNNING; backend health mode=live). Frontend runs Vite DEV (`yarn start`, HMR) — `.env` changes need `sudo supervisorctl restart frontend`; code edits hot-reload.
...
- **.env:** updated `frontend/.env` `VITE_API_BASE_URL` from the stale `5c680fc7-…` host to `https://reseller-panel-9.preview.emergentagent.com` (used for Google OAuth redirects; API calls already use same-origin).
...
REAL pod URL = `https://reseller-panel-9.preview.emergentagent.com` (from supervisor
...
Live pod URL: `https://reseller-panel-9.preview.emergentagent.com`
...
- Pod URL used for APP_URL/FRONTEND_URL/GOOGLE redirects/VITE_API_BASE_URL = `https://reseller-panel-9.preview.emergentagent.com` (user's pasted `.env` had a stale `5c680fc7…` URL — kept in CORS_ORIGIN only).
...
- Preview URL: https://reseller-panel-9.preview.emergentagent.com
...
- URL vars (APP_URL/FRONTEND_URL/CORS/GOOGLE redirects/VITE_API_BASE_URL) were pointed to the ACTUAL pod preview URL `https://reseller-panel-9.preview.emergentagent.com` (user's .env had a stale `5c680fc7-...` URL that doesn't map to this pod).
...
- Pod preview URL is now `https://reseller-panel-9.preview.emergentagent.com` (user's pasted `5c680fc7-...` URL was stale → all URL configs use the live pod URL).
...
- ACTUAL pod preview URL = `https://reseller-panel-9.preview.emergentagent.com` (from env `preview_endpoint`/HOSTNAME). User's pasted `5c680fc7-...` URL is STALE → all URL vars (APP_URL/FRONTEND_URL/CORS/GOOGLE redirects/VITE_API_BASE_URL) point to the live 2b950aee pod URL.
...
- NEW live pod preview URL = `https://reseller-panel-9.preview.emergentagent.com` (user pasted a STALE `5c680fc7-...` URL in their .env -> repointed APP_URL/FRONTEND_URL/CORS/GOOGLE redirects to the live pod URL).
...
- NEW pod preview URL: `https://reseller-panel-9.preview.emergentagent.com` (old `5c680fc7-...` is stale; kept in CORS only).
...
- ACTUAL pod preview URL = `https://reseller-panel-9.preview.emergentagent.com` (from env `preview_endpoint`/HOSTNAME). User's pasted `5c680fc7-...` URL is STALE -> all URL vars (APP_URL/FRONTEND_URL/CORS/GOOGLE redirects/VITE_API_BASE_URL) point to the live d1d1fa08 pod URL.
...
- ACTUAL live pod preview URL = `https://reseller-panel-9.preview.emergentagent.com` (from env `preview_endpoint`). User's pasted `5c680fc7-...` URL is STALE → all URL vars (APP_URL/FRONTEND_URL/CORS/GOOGLE redirects/VITE_API_BASE_URL) repointed to the live c9b751a1 URL.
...
- ACTUAL pod preview URL = `https://reseller-panel-9.preview.emergentagent.com` (from env `preview_endpoint`/HOSTNAME). User's pasted `5c680fc7-...` is STALE → all URL vars (APP_URL/FRONTEND_URL/CORS/GOOGLE redirects/VITE_API_BASE_URL) repointed to the live 98499cc0 URL.
...
- ACTUAL pod preview URL = `https://reseller-panel-9.preview.emergentagent.com` (user's pasted `5c680fc7-...` URL is STALE → all URL vars point to the live df85add3 pod URL; 5c680fc7 + localhost also added to CORS_ORIGIN, though CORS uses origin:true anyway).
