...
## Hostinger-style animated hero (2026-06) — IMPLEMENTED, NOT YET REBUILT/TESTED
- Installed `motion@13.2.0` (Framer Motion for React 19; import from `motion/react`, aliased to `Motion` because eslint has no eslint-plugin-react and `no-unused-vars` ignores only `^[A-Z_]`).
- NEW `frontend/src/components/home/HeroShowcase.jsx`: right-column animated storyboard carousel that auto-advances (3.8s) through 4 scenes inside a browser-chrome window — Domains (search + availability rows), Hosting (cPanel + animated metric bars), VPS·RDP (specs + terminal deploy line), Manage (DNS records + wallet). Clickable segmented tabs with a motion progress underline, pause-on-hover, two floating badges (WHOIS/jurisdiction) with idle float, full `useReducedMotion` support, data-testids (`hero-showcase`, `hero-showcase-tab-*`).
- `HomeRedesign.jsx` Hero restructured (Hostinger-style, search kept as primary CTA): star-rating pill next to eyebrow, headline split on first comma with the trailing clause as an indigo gradient highlight (works EN/ES/FR), staggered motion entrance on left column, trust-note line under search, `<HeroShowcase/>` replaces the old static image. Rest of page untouched.
- Locale keys added to en/es/fr `home`: `rating`, `trustNote`, `showcase{live,tabs,domains,hosting,servers,manage}`.
- STATUS: lint clean. NOT rebuilt yet — frontend serves the PRODUCTION build via `vite preview` (`/app/frontend/.prod` present), so changes are invisible in preview until `cd /app/frontend && yarn build && sudo supervisorctl restart frontend`. Then smoke-test the hero (screenshot) + optionally run frontend testing_agent for the carousel/tabs/reduced-motion.
