...
- ✅ **VERIFIED (frontend testing agent)** — crypto applies to ALL products via the wallet: (1) wallet-page top-up (USDT-TRC20 + ETH addresses, QR, $10-min validation, desktop+mobile); (2) checkout crypto — demo (wallet $0) added a domain → cart showed "Top up $39 & pay" → same crypto modal opened → address+QR generated. No crypto sent; buyer's $60 preserved. (TEST 3 buyer pay-step was only partial due to a browser session hiccup on user-switch — not a functional issue; buyer balance confirmed $60.)
