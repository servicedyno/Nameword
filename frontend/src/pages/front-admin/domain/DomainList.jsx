...
// Owned-domain portfolio (Nomadly reseller shape: domain / status / mode / ns_choice / created_at / order_id).
const DomainList = () => {
  const { t } = useLanguage();
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="min-w-0">
            ...
            {d.registrar && <p className="nw-mono mt-0.5 !tracking-normal !normal-case text-[11px]">{d.registrar}</p>}
          </div>
        );
      },
    },
        ...
        return (
          <div className="flex w-full items-center justify-end gap-2">
            <Link to={`/dns-manager?domain=${encodeURIComponent(d.domain || "")}`} className="nw-btn-secondary nw-btn-sm" data-testid={`domain-manage-dns-${d.domain}`}>
              <FiSettings size={14} /> Manage DNS
            </Link>
            {d.order_id && (
              <Link to={`/checkout/success/${d.order_id}`} className="nw-btn-ghost nw-btn-sm" title="View receipt" data-testid={`domain-receipt-${d.domain}`}>
                <FiFileText size={14} />
              </Link>
            )}
          </div>
