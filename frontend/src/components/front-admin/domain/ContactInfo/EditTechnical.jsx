...
const EditTechnical = ({ data, onSave, onCancel, currentDomain }) => {
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        ...
        console.log("[EditTechnical] Saving contact data:", contactData);
        console.log("[EditTechnical] Domain:", currentDomain.websiteName);
        console.log("[EditTechnical] Existing data handle:", data?.handle);

        try {
            await domainAPI.updateDomainContact(currentDomain.websiteName, data?.handle || 'technical', 'technical', contactData);
            console.log("[EditTechnical] Contact updated successfully");
            showAlert(t.admin.technicalContactUpdated, { duration: 2500, type: 'success' });
            onSave();
        } catch (error) {
            console.error("[EditTechnical] Error saving contact:", error);
