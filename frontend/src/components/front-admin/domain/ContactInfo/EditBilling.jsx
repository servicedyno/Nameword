...
const EditBilling = ({ data, onSave, onCancel, currentDomain }) => {
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        ...
        console.log("[EditBilling] Saving contact data:", contactData);
        console.log("[EditBilling] Domain:", currentDomain.websiteName);
        console.log("[EditBilling] Existing data handle:", data?.handle);

        try {
            await domainAPI.updateDomainContact(currentDomain.websiteName, data?.handle || 'billing', 'billing', contactData);
            console.log("[EditBilling] Contact updated successfully");
            showAlert(t.admin.billingContactUpdated, { duration: 2500, type: 'success' });
            onSave();
        } catch (error) {
            console.error("[EditBilling] Error saving contact:", error);
