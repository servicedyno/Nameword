...
const EditRegistrantContact = ({ data, onSave, onCancel, currentDomain }) => {
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        // Handle both string and object formats for currentDomain
        const domainName = typeof currentDomain === 'string' ? currentDomain : currentDomain?.websiteName;
        ...
        console.log("[EditRegistrantContact] Saving contact data:", contactData);
        console.log("[EditRegistrantContact] Domain:", domainName);
        console.log("[EditRegistrantContact] Existing data handle:", data?.handle || 'registrant');
        
        try {
            await domainAPI.updateDomainContact(domainName, data?.handle || 'registrant', 'registrant', contactData);
            console.log("[EditRegistrantContact] Contact updated successfully");
            showAlert(t.admin.registrantContactUpdated, { duration: 2500, type: 'success' });
            onSave();
        } catch (error) {
            console.error("[EditRegistrantContact] Error saving contact:", error);
...
export default EditRegistrantContact
