...
const Technical = ({ data, onRefresh, onEdit }) => {
...
${t.admin.company} ${data?.company_name || 'N/A'}`;
    ...
    const handlePaste = async () => {
        ...
        try {
            ...
            console.log("[Technical Paste] Using update API for HostBay");
            console.log("[Technical Paste] Contact data to update:", contactData);
            console.log("[Technical Paste] Domain:", currentDomain.websiteName);
            console.log("[Technical Paste] Existing handle:", data?.handle || 'technical');

            try {
                // Always use update API - backend will merge with existing data
                await domainAPI.updateDomainContact(
                    currentDomain.websiteName,
                    data?.handle || 'technical',
                    'technical',
                    contactData
                );
    ...
    return (
        <div>
            {/* <p className="info-card-title px-5 py-4">Technical Contact</p>
