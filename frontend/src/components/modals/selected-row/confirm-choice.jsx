...
const ConfirmChoice = ({ onClose, onBack, selectedDomains = [], action = 'enable', turnOffReason = {} }) => {
    const { t } = useLanguage();
    ...
    const analyzeDomains = () => {
        ...
        const targetValue = action === 'enable';
        ...
        selectedDomains.forEach((domain) => {
            ...
            const currentValue = normalizeAutoRenew(domain.autorenew);
    ...
    const handleApply = async () => {
        ...
        try {
            ...
            const response = await domainAPI.bulkManageAutoRenewal({
                domains: domainNames,
                autorenew: action === 'enable',
                // Include reason and description when disabling auto-renewal
                ...(action === 'disable' && turnOffReason ? {
                    reason: turnOffReason.reason,
                    description: turnOffReason.description,
                } : {}),
            });

            if (response.success) {
                setResults(response.data);
                showAlert(response.message || t.admin.autoRenewalUpdatedSuccess, { type: 'success' });
                ...
                showAlert(response.message || t.admin.failedToUpdateAutoRenewal, { type: 'error' });
            }
        } catch (error) {
            const errorMsg = error?.response?.data?.message || error.message || t.admin.failedToUpdateAutoRenewal;
                    ...
                    <div className="modal-dialog">
                        ...
                        <div className="flex flex-col gap-2">
                            <h2 className="modal-title">
                                {action === 'enable' ? t.admin.autoRenewalEnabledTitle : t.admin.autoRenewalDisabledTitle}
                            </h2>
                        </div>
            ...
            <div className="flex items-center justify-center w-full h-full">
                <div className="modal-dialog">
                    ...
                    <div className="space-y-2 mt-4">
                        {alreadyCompleted > 0 && (
                            <p className="text-13 text-primary dark:text-gray-400 font-medium flex items-center gap-2">
                                <IoCheckmarkOutline size={14} className="text-tealdark" />
                                {action === 'enable'
                                    ? t.admin.domainsAlreadyHaveAutoRenewalEnabled.replace("{count}", alreadyCompleted).replace(/{plural}/g, alreadyCompleted === 1 ? '' : 's')
                                    : t.admin.domainsAlreadyHaveAutoRenewalDisabled.replace("{count}", alreadyCompleted).replace(/{plural}/g, alreadyCompleted === 1 ? '' : 's')
                                }
                            </p>
                        )}
                        ...
                        {willBeAffected === 0 && alreadyCompleted > 0 && (
                            <p className="text-13 text-secondary font-medium">
                                {t.admin.allDomainsAlreadyHaveAutoRenewalSetting}
                            </p>
                        )}
                    </div>
                    ...
                    <div className="mt-5 flex justify-end gap-2 admin-btn">
                        ...
                        >
                            {loading ? t.admin.processing : action === 'enable' ? t.admin.enableAutoRenewalButton : t.admin.disableAutoRenewalButton}
                        </button>
                    </div>
                </div>
            </div>
