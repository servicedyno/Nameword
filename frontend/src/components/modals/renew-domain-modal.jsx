...
const addYears = (dateInput, years) => {
  if (!dateInput || !years) return null;
...
const RenewDomainModal = ({
  ...
  // Promocode state (same as /cart)
  const [promoCode, setPromoCode] = useState("");
  const [appliedPromoCode, setAppliedPromoCode] = useState(null);
  const [promoDiscount, setPromoDiscount] = useState(0);
  const [validatingPromo, setValidatingPromo] = useState(false);

  // Tax state (same as /cart)
  ...
  // Load countries (same as PaymentOrderSummary)
  useEffect(() => {
  ...
  // Auto-detect country from IP (same as /cart)
  useEffect(() => {
  ...
  // Fetch tax rate when country changes
  useEffect(() => {
    ...
    const fetchPrices = async () => {
      ...
      const requests = TERMS.map(async (term) => {
        try {
          ...
          const descriptionList = response?.responseData?.["0"] || [];
          const baseEntry = Array.isArray(descriptionList)
            ? descriptionList.find((entry) => {
              const match = entry?.description?.match(/for\s+(\d+)\s+year/i);
              return match && Number(match[1]) === term;
            })
            : null;

          const basePriceMatch =
            baseEntry?.description?.match(/is\s+([\d.]+)/i);
          const basePrice = basePriceMatch
            ? Number.parseFloat(basePriceMatch[1])
            : null;

          const totalPriceRaw =
            response?.responseData?.renewalfee ??
            response?.responseData?.registrationFee ??
            basePrice ??
            0;
          const totalPrice = Number.parseFloat(totalPriceRaw) || 0;
          ...
          console.error("Failed to fetch renewal pricing:", err);
          return {
            term,
            basePrice: null,
            totalPrice: 0,
      ...
      if (firstError) {
        ...
        const message = normalizeErrorMessage(firstError.error, fallbackMessage);
        setError(message);
        showAlert(message, { duration: 3000, type: "fail" });
      }

      const sanitized = results
        .map(({ term, basePrice, totalPrice }) => ({
          term,
          basePrice,
          totalPrice,
        }))
  ...
  const newExpiration = useMemo(() => {
    const extended = addYears(currentExpiration, selectedTerm);
    return extended ? formatDate(extended) : "—";
  }, [currentExpiration, selectedTerm]);
  ...
  const handleRenewCheckout = async () => {
    ...
    if (!selectedOption) {
      showAlert(t.admin.selectRenewalTerm || "Please select a renewal term to continue.", {
        duration: 2500,
        type: "fail",
      });
      return;
    ...
    try {
      const response = await domainAPI.getDomainRenewDynoCheckoutUrl({
        domain: domainName,
        domainNameId,
        provider: pricingProvider,
      ...
      const backendMsg = normalizeErrorMessage(
        ...
        t.admin.failedToStartRenewalCheckout || "Failed to start renewal checkout."
      );
        ...
        <div className="modal-dialog">
          ...
          <div className="flex flex-col gap-2">
            ...
            <h2 className="modal-title">
              {t.admin.renewDomainRegistration || "Renew your Domain Registration"}
          ...
          <div className="flex gap-2 justify-between items-center w-full mt-4">
            <div className="flex flex-col gap-1 w-full">
              ...
              {isLoading && (
                <div className="py-7 px-5">
                  <p className="text-secondary text-sm">
                    {t.admin.loadingRenewalOptions || "Loading renewal options..."}
                  </p>
                </div>
              )}
              ...
              {error && (
                <p className="text-warning text-xs px-4">{error}</p>
              )}
            </div>
          </div>
            ...
            <div className="flex gap-5 flex-col w-full">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  <IoChevronDown size={14} className={`transition-transform ${showPromocode ? "rotate-180" : ""}`} />
                </button>
                {showPromocode && (
                  <>
                    {!appliedPromoCode ? (
                ...
                <div className="flex justify-start my-2">
                  <button
                    ...
                    disabled={
                      isCheckoutLoading || !selectedOption || isLoading
                    }
...
RenewDomainModal.propTypes = {
  onClose: PropTypes.func.isRequired,
  domainName: PropTypes.string,
  pricingProvider: PropTypes.string,
  currentExpiration: PropTypes.oneOfType([
    PropTypes.string,
    PropTypes.number,
    PropTypes.instanceOf(Date),
  ]),
  domainNameId: PropTypes.oneOfType([
    PropTypes.string,
    PropTypes.number,
  ]),
};

RenewDomainModal.defaultProps = {
  domainName: "",
  pricingProvider: "openprovider",
